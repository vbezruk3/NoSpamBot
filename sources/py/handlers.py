...
async def echoHelp(message: Message):
    await message.answer(text = commands.callCommand('/addWord', message.text.replace('/addWord ', ''), message.from_user.id))

@dp.message_handler(commands = ['removeWord'])
async def echoHelp(message: Message):
    await message.answer(text = commands.callCommand('/removeWord', message.text.replace('/removeWord ', ''), message.from_user.id))
